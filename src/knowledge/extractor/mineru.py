...
class MinerUExtractor(BaseExtractor):
    ...
    def __init__(
        ...
        *,
        ...
        self.api_url = config.mineru_api_url if api_url is None else api_url
        self.api_key = config.mineru_api_key if api_key is None else api_key
        ...
        **params: Any,
        ...
        if not status.get("available"):
            return _failure_result(
                self.service_name(),
                path,
                str(status.get("error") or "MinerU API is unavailable."),
                status,
            )
        ...
        try:
            ...
            return _failure_result(
                self.service_name(),
                path,
                f"MinerU API extraction failed: {exc}",
                {
                    "stage": "extract",
                    "engine": "mineru-api",
                    "backend": backend,
        ...
        if not content.strip():
            return _failure_result(
                self.service_name(),
                path,
                "MinerU API completed but no extractable content was found.",
                {"stage": "extract", "engine": "mineru-api", "backend": backend},
            )
    ...
    async def check_status(self, **params: Any) -> dict[str, Any]:
        ...
        return await _check_http_api(
            ...
            timeout=self._timeout(params),
            missing_config_key="mineru_api_url",
        )
    ...
    def _api_url(self, params: dict[str, Any]) -> str:
        api_url = str(params.get("mineru_api_url") or self.api_url).strip()
        return _resolve_mineru_api_url(api_url) if api_url else ""

    def _api_key(self, params: dict[str, Any]) -> str:
        return str(params.get("mineru_api_key") or self.api_key).strip()
    ...
    def _backend(self, params: dict[str, Any]) -> str:
        return str(params.get("mineru_backend") or params.get("backend") or self.backend)
