  ...
  return (
    <div className="flex flex-wrap gap-2 mb-2">
      {images.map((image, index) => (
        <div
          className={`glass-effect-sm flex max-w-[132px] items-center gap-1.5 rounded-full bg-card-background p-0.5 pr-1.5 ${image.uploading ? "opacity-70" : ""}`}
      ...
      {attachments.map((file, index) => {
        ...
        return (
          <div
            className={`glass-effect-sm flex max-w-[132px] items-center gap-1.5 rounded-full bg-card-background p-0.5 pr-1.5 ${file.uploading ? "opacity-70" : ""}`}
